...
const HQPOutput & Solver_HQP_qpoases::solve(const HQPData & problemData) {
	...
	for (int i = 0; i < level_num - 1; i++) {
		...
		m_ci0[c_level].tail(m_slack) = 100000000.0 * VectorXd(m_slack).setOnes();
		m_ci1[c_level].tail(m_slack) = -100000000.0 * VectorXd(m_slack).setOnes();
		...
		if (m_status == SUCCESSFUL_RETURN)
		{
			...
			// 	cout << "M_CI" << m_CI[c_level] << endl;
			// 	cout << "M_CE" << m_CE[c_level] << endl;
			// 	cout << "x_sol" << x_sol[c_level].transpose() << endl;
			//	getchar();
