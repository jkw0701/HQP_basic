...
void VRepBridge::read()
{
	...
	simxGetObjectVelocity(clientID_, endEffector, data3, data4, simx_opmode_streaming);
	EE_vel_.linear() << data3[0], data3[1], data3[2];
	EE_vel_.angular() << data4[0], data4[1], data4[2];
...
void VRepBridge::getHandle()
{
	...
	simxErrorCheck(simxGetObjectHandle(clientID_, "panda_link7_respondable2", &endEffector, simx_opmode_oneshot_wait));
